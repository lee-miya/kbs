...
> 这是知识库的加载入口。新对话先读本文件，再按需拉取具体分册。
> 最近更新：2026-08-09（v1.3：周更对齐；合并 20260809 批次；PHP/Java/综合/方法论回链）

## 一、分册索引

| 分册文件 | 适用场景 | 核心内容 | 更新 |
|---|---|---|---|
| PHP_代码审计.md | PHP Web 应用（CMS、论坛、商城） | SQLi 各形态、上传、包含、反序列化、弱类型、认证逻辑；CI4 上传时效 | 2026-08-09 |
| Java_代码审计.md | Java Web（Spring、Struts、Shiro 体系） | 反序列化与利用链、表达式注入、SSTI、内存马、组件速查；Tomcat fail-open | 2026-08-09 |
| Python_代码审计.md | Python Web 与脚本 | pickle/yaml 反序列化、SSTI、命令执行、解压穿越 | 2026-07-31 |
| JavaScript_Node_代码审计.md | Node.js 后端 | 原型链污染、命令注入、NoSQL 注入、供应链 | 2026-07-31 |
| Go_代码审计.md | Go 服务与工具 | 命令执行、SQL 拼接、SSRF、路径处理、并发逻辑 | 2026-07-31 |
| C_CPP_内存破坏与Fuzzing.md | C/C++ 二进制目标 | 内存破坏模式、AFL++ fuzzing 全流程、崩溃归因到 PoC | 2026-07-31 |
| 通用审计方法论.md | 所有项目开工前必读 | 审计三路径、patch diffing、fail-open 纪律、漏洞链组合 | 2026-08-09 |
| 综合分册_AI漏洞挖掘与态势.md | 借鉴 AI 挖洞方法、校准审计优先级、审 AI 应用 | AI 挖洞体系、Agent/低代码攻击面、窗口期态势 | 2026-08-09 |
...
| 借鉴 AI 挖洞方法 / 校准优先级 / 审 AI 应用 | 综合分册 + 通用方法论 |
...
- 本索引随任何分册更新而同步更新"更新"列。
- 速查表只放 TOP 高频模式；新增模式先加分册，足够高频再进本表。
- 双周维护的增量批次原料留档于 `增量池/批次日期/`，审核合并进分册后原料保留不删，供溯源；使用中不直接加载原料，只加载分册。
- **自 2026-08-09 起维护节奏改为每周**（与红队 `12_每周情报` 对齐），详见仓库根目录 `维护/每周更新SOP.md`。
