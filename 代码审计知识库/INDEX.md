...
> 最近更新：2026-08-09（v1.4：导航体例打磨；方法论/综合前置；与红队交叉引用）

## 一、分册索引

| 分册文件 | 适用场景 | 核心内容 | 更新 |
|---|---|---|---|
| [通用审计方法论.md](通用审计方法论.md) | 所有项目开工前必读 | 审计三路径、patch diffing、fail-open、漏洞链 | 2026-08-09 |
| [综合分册_AI漏洞挖掘与态势.md](综合分册_AI漏洞挖掘与态势.md) | AI 挖洞 / 审 AI 应用 / 定优先级 | 工具方法论、Agent 攻击面、窗口期态势 | 2026-08-09 |
| [PHP_代码审计.md](PHP_代码审计.md) | PHP Web（CMS、论坛、商城） | SQLi、上传、包含、反序列化、弱类型；CI4 时效 | 2026-08-09 |
| [Java_代码审计.md](Java_代码审计.md) | Java Web（Spring/Struts/Shiro） | 反序列化、表达式/SSTI、内存马；Tomcat fail-open | 2026-08-09 |
| [Python_代码审计.md](Python_代码审计.md) | Python Web 与脚本 | pickle/yaml、SSTI、命令执行、解压穿越 | 2026-08-09 |
...
| Web 渗透手法（非源码） | [`../红队渗透知识库/02_Web渗透`](../红队渗透知识库/02_Web渗透) |
...
- 速查表只放 TOP 高频模式；新增模式先加分册「时效条目」，足够高频再进本表。
- 周增量原料留档于 `增量池/YYYYMMDD/`，合并进分册后原料保留供溯源；**使用时只加载分册，不直接加载原料**。
- 节奏与红队 `12_每周情报` 对齐（每周），见 [`../维护/每周更新SOP.md`](../维护/每周更新SOP.md)；体例见 [`../维护/文档体例约定.md`](../维护/文档体例约定.md)。
