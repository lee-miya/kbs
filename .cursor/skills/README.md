...
> 项目级 skill：让 Cursor/兼容 Agent 按 KBS 分册作战。  
...
1. **只提交本表三枚 skill**。第三方攻防库（Claude-Red、BugHunter、OSINT 等）若要试用，只装到本机 `~/.cursor/skills/`，**禁止**拷进本目录或 git。
...
```
always-on 规则（.cursor/rules/red-team-operator.mdc）
        ↓
本目录 skill（短循环）
        ↓
