...
> 原则：先确认「真拦截还是假拦截」（换出口/降速/单参数）；再按层绕过。完整生产级混淆载荷不入库。

| 层 | 常见拦法 | 绕过思路 |
| --- | --- | --- |
| 速率 / IP 信誉 | 短时封禁、验证码 | 降 `--delay`、换出口、分散参数探测 |
| 关键字签名 | `union select`、`../`、`{{` | 注释拆分、大小写、编码（URL/Unicode/双重）、等价函数 |
| 协议解析差异 | 只检 URL 不检 body；只检第一个参数 | 换方法、重复参数、分块传输、Trailer（结合 02-01 CRLF/desync） |
| Cookie / 头 | 拦 UA、拦某头 | 最小化请求头；业务必须头再逐个加回 |
| 文件上传 | 拦扩展名/Content-Type | 见 §1 防护层级表；先过 WAF 再过应用校验 |

与边界关系：很多站点前有 **ADC/WAF**；打点时同步评估管理口是否误暴露（回链 **09-03**），有时绕站不如绕设备。
...
- [ ] 能说明「WAF 拦 SQL 关键字」时至少两种等价改写思路，并知道何时该转打边界设备。
