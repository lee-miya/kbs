...
2. **本地 Ext（硬性）**：读取 `代码审计知识库_Ext/` 与 `红队渗透知识库_Ext/`，把新内容改写进主库；两目录已 gitignore，**禁止提交**。
3. 产出必须符合 [`红队渗透知识库/12_每周情报/README.md`](../../../红队渗透知识库/12_每周情报/README.md) 模板（含 3.5–**3.9**）。
4. 不入库：真实目标/凭证、完整 exploit、完整免杀载荷、完整 PhaaS、第三方 skill 原文、两个 `_Ext` 目录原文。
...
- 路线图勾选；关注源可追加高产仓库。
- `git status` 排除 `AGENTS.md`、`AGENTS.local.md`、`.firecrawl/`、个人笔记、**`代码审计知识库_Ext/`、`红队渗透知识库_Ext/`**；按仓库约定提交推送。
